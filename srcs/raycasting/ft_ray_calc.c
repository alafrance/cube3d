...
/*   Updated: 2021/02/16 15:56:08 by alafranc         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	calculate_distance(t_data data, t_ray ray_data, int i)
{
	...
	ray_data.rayDir[0] = ray_data.dir[0] + ray_data.plane[0] * camera_x;
	ray_data.rayDir[1] = ray_data.dir[1] + ray_data.plane[1] * camera_x;
...
void	ft_calculate_delta_dist(t_ray *ray_data)
{
	...
	{
		ray_data->deltaDist[0] = 0;
		ray_data->deltaDist[1] = 1;
	}
	else if (ray_data->rayDir[0] == 0)
	{
		ray_data->deltaDist[0] = 1;
		ray_data->deltaDist[1] = 0;
	}
	else
	{
...
void	ft_calculate_texture(t_ray *ray_data, t_img texture, t_data data)
{
	...
	if ((ray_data->side && ray_data->rayDir[1] < 0)
		|| (!ray_data->side && ray_data->rayDir[0] > 0))
